...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("SELECT * FROM cadastro WHERE (email = :usuario OR nome_user = :usuario)");
    $stmt->bindParam(":usuario", $usuario);
    $stmt->execute();
    $dados = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($dados && $senha == $dados["senha"]) {
        $_SESSION['id_user'] = $dados['id_user']; // Essencial para o carrinho
...
<body>
    <header class="topo">
        <div class="topo-esquerda">
            <a href="../Index/index.php"><img class="logo" src="../imagens/logo.png"></a>
        </div>
        <div class="topo-direita">
            <a href="../Sac/Suporte.php"><button class="btn-login">Suporte</button></a>
        </div>
    </header>
    <div class="tela">
        <div class="h1box">
